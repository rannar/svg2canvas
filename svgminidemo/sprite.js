...
//		console.log( st.interval);
     ...
     * @method createTag
     */
    // svg.CreateElement = function(tag) {
    ...
    st.initRoot = function() {
        ...
        st.root = svg.CreateElement('svg',{
            'width': (rect.xm - rect.xi),
            'height': (rect.ym - rect.yi),
        ...
        // st.scene.addChild(st.root);
        // document.body.addChild(st.scene);
    };
    ...
    st.processFont = function(j) {
        var font = st.CreateElement('font');
        font.setAttribute('id', 'ft' + j.id);
        font.setAttribute("horiz-adv-x", 1024);
    
        var fontFace = st.CreateElement('font-face');
        ...
        for (var i = 0, k = j.fo.length; i < k; i++) {
            ...
            var glyph = st.CreateElement('glyph');
            ...
            for (var i = 0; j.rs[i]; i++) {
                ...
                if (j.tt == "html") {
                    ...
                    if (i == 0) {
                        ...
                        var text_foreignobject = svg.CreateElement('foreignObject');
                        ...
                        var text_g = svg.CreateElement('g');
                        text_g.setAttribute('transform', j.tf);
                        text_g.setAttribute('translate', r.translate);
                        text_g.addChild(text_foreignobject);
                        text_foreignobject.addChild(text_body);
                        text_body.addChild(text_div);
                        text_div.addChild(text_p);
                        text_p.addChild(text_span);
                        g.addChild(text_g);
                        
                        var lastSpan = text_span;
                    } else {
                        lastSpan.addChild(text_span);
                        lastSpan = text_span;
                    }
                } else {
    ...
    st.processButton = function(tag) {
        var button = svg.CreateElement('g');
        var defs = svg.CreateElement('defs');
        var childsHolder = svg.CreateElement('g');
...
//			console.log(st.defineShapes);
    ...
    st.filterElement = function(g, filters) {
        ...
        this.defs = g.children[0];
        this.target = g.children[1];
        this.id = 'filter_' + g.getAttribute('id');
        this.base = 0;
        this.filters = filters;
        this.element = svg.CreateElement('filter'); // 创建filter节点
...
//		    console.log(this.clipDepthId);
    ...
    */
    st.sprite.prototype.createSimpleClip = function(shape) {
        ...
        var cp = svg.CreateElement('clipPath');
    ...
    */
    ...
    st.start = function() {
        ...
        st.timer = setTimeout(function() {
            mainMovie.showFrame();
            // console.log('st.root.innerHTML',st.root.innerHTML);
            // debugger;
            svg.loadXml('<svg>'+st.root.innerHTML+'</svg>');
		...
		function _StartAndStop() {
			...
			control.id = "control";
			document.body.addChild(control);
			if(control) {
